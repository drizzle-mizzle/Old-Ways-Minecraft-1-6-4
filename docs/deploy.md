...
- **С вашей машины** понадобятся два файла, которых нет в репозитории:
  OptiFine и Rei's Minimap из `mirror/mods`. Они чужие, раздавать их мы
  не вправе, а скачать их скриптом неоткуда. Всё остальное — включая
  Windows-лаунчер — собирается прямо на сервере.
...
Наш coremod — он правит адреса авторизации уже в игре — собирается на месте:

```bash
sudo apt-get install -y openjdk-8-jdk-headless
python3 tools/build_coremod.py
```

А два чужих мода придётся однажды принести со своей машины: скачать их
скриптом неоткуда. Команда выполняется **на ней**, в PowerShell:

```powershell
cd C:/Users/flower/Desktop/Server
scp mirror/mods/OptiFine_1.6.4_HD_U_D1.jar "mirror/mods/[1.6.4]ReiMinimap_v3.4_01.zip" oldways@ВАШ_IP:~/old-ways/mirror/mods/
```

В итоге в `mirror/mods` должно лежать три файла: два перенесённых и
`oldways-auth-1.0.jar`, собранный здесь же.
...
Сам лаунчер эта команда не собирает — он попадёт в `dist/launcher/` на шаге 9,
с вашей Windows-машины. Скрипт об этом напомнит, пока каталога нет.
...
Игрок скачивает один exe, внутри которого лежит и Java. Собирается он прямо
здесь: Windows-машина не нужна, хватает кросс-компилятора.

```bash
sudo apt-get install -y openjdk-8-jdk-headless mingw-w64
python3 tools/fetch_jre.py          # виндовая Java 8 — она вкладывается в exe
echo ВАШ_IP > launcher/ADDRESS
python3 tools/publish_exe.py
```

Три пояснения к этим строкам:

- **Java 8 нужна дважды и разная.** JDK из пакета собирает jar — он работает
  на сервере. А `fetch_jre.py` качает **виндовый** рантайм: он поедет внутрь
  exe и будет работать у игрока. Скачанное сверяется по sha256 с тем, что
  сообщил Adoptium.
- **`launcher/ADDRESS`** — адрес, который окажется в поле «Адрес сервера» при
  первом запуске, чтобы игроку не вводить его руками. Значение по умолчанию:
  у того, кто лаунчер уже запускал, сохранённый адрес не трогается.
- **`publish_exe.py`** поднимает младшую цифру версии, собирает exe и кладёт
  его вместе с jar в `dist/launcher` — туда, откуда их раздаёт сервис. Сборка
  печатает, что получилось: `версия 0.3.4, адрес по умолчанию ВАШ_IP`.

Собирать можно и на Windows — тогда понадобятся распакованные рядом `jdk8/`,
`jre8/` и `mingw64/` (ссылки в шапке `launcher/package.py`), а готовое
переносится на сервер: `scp -r dist/launcher oldways@ВАШ_IP:~/old-ways/dist/`.
...
**Ссылка для игроков** — прямо с сервера, ничего больше настраивать не нужно:

```
http://ВАШ_IP:8080/dist/launcher/exe
```

Браузер скачает `Old Ways.exe`. Дальше игроку достаточно запустить его:
адрес уже вписан, останется ввести логин и пароль.
...
Про плагины есть тонкость. Jar-ники образ раскладывает в том при каждом старте,
а вот конфиги — только когда том пустой: иначе пересборка затирала бы всё, что
вы правили руками. Поэтому **новому** плагину, который приехал в уже живой том,
конфиг из репозитория надо положить один раз самому, иначе он сгенерирует свой:

```bash
docker cp plugins/CraftBook/config.yml mc164:/data/plugins/CraftBook/config.yml
docker exec -u root mc164 chown 2000:2000 /data/plugins/CraftBook/config.yml
